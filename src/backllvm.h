...
void create_printstr() {
	if (printstr)
		return;
	std::vector<Type*> arg_types;
	arg_types.push_back(Type::getInt8PtrTy(ctx));
	FunctionType *ftype = FunctionType::get(Type::getVoidTy(ctx), arg_types, false);
	printstr = Function::Create(ftype, Function::ExternalLinkage, "printstr", module);
	printstr->setCallingConv(CallingConv::C);
}
...
void generate_object(const std::string &outfilename) {
